...
let INDEX=[],CHAPTERS={},activeGold='SKM-SPN-0001',PRESCRIBER={},REASONING={},CLINICALMASTER={},FINALMASTER={};let deferredPrompt=null;
...
async function loadExpansionInBackground(){
 const total=EXPANSION_FILES.length;
 let loaded=0;
 const status=document.getElementById('goldSearchMeta');
 const concurrency=24;
 for(let i=0;i<EXPANSION_FILES.length;i+=concurrency){
   const batch=EXPANSION_FILES.slice(i,i+concurrency);
   ...
   if(status) status.textContent=`Loading Gold Chapters… ${Math.min(loaded,total)} / ${total}`;
   refreshCoreUI();
   await new Promise(r=>setTimeout(r,0));
 }
 if(status) status.textContent=`${Object.keys(CHAPTERS).length} Gold Chapters ready`;
 const pb=document.querySelector('.prescriberBand small');
 if(pb) pb.textContent='Prescriber data loaded for '+Object.keys(PRESCRIBER).filter(id=>CHAPTERS[id]).length+' core chapters • 550-chapter library ready';
}

async function load(){
 INDEX=await fetch('./data/master-index.json',{cache:'no-store'}).then(r=>{
   if(!r.ok) throw new Error('data/master-index.json HTTP '+r.status);
   return r.json();
 });
 await loadGoldIndex();

 // Load only the five core chapters first so the app becomes usable immediately.
 await Promise.all(CORE_FILES.map(async f=>{
   try{const d=await fetchJSON(f);CHAPTERS[d.id]=d}catch(e){console.warn('Core chapter load failed',f,e)}
 }));

 // Prescriber data is attached BEFORE first render.
 try{
 ...
 try{
   const cr=await fetchJSON('data/clinical-reasoning-core-v1.json');
   REASONING=cr?.chapters||{};
   Object.keys(REASONING).forEach(id=>{if(CHAPTERS[id])CHAPTERS[id].clinicalReasoning=REASONING[id]});
 }catch(e){console.warn('Clinical reasoning load failed',e)}
 try{
   const cm=await fetchJSON('data/clinical-master-sciatica-v2.json');
   CLINICALMASTER=cm?.chapters||{};
   Object.keys(CLINICALMASTER).forEach(id=>{if(CHAPTERS[id])CHAPTERS[id].clinicalMaster=CLINICALMASTER[id]});
 }catch(e){console.warn('Clinical master load failed',e)}
 try{
   const fm=await fetchJSON('data/clinical-master-sciatica-final-v1.json');
   const ch=fm?.chapter;
   if(ch?.id&&CHAPTERS[ch.id]){FINALMASTER[ch.id]=ch;CHAPTERS[ch.id].finalClinicalMaster=ch;}
 }catch(e){console.warn('Final clinical master load failed',e)}
 init();

 const pb=document.querySelector('.prescriberBand small');
 if(pb) pb.textContent='Prescriber data loaded for '+Object.keys(PRESCRIBER).filter(id=>CHAPTERS[id]).length+' core chapters • remaining Gold Chapters loading in background';

 // Load the remaining chapters without blocking Disease Library or Core Prescriber display.
 loadExpansionInBackground().catch(e=>console.warn('Background Gold loading failed',e));
}
...
function refreshCoreUI(){
 if($('stats')) $('stats').innerHTML=[['Master index',INDEX.length],['Legacy V1 slots',INDEX.filter(x=>x.sourceLayer==='Legacy V1').length],['Reference chart',INDEX.filter(x=>x.sourceLayer==='Anukta 100 Chart').length],['Gold chapters',Object.keys(CHAPTERS).length]].map(([t,n])=>`<div class=stat><b>${n}</b><span>${t}</span></div>`).join('');
 if($('goldSelect')){
   const current=activeGold;
   ...
   if(CHAPTERS[current]) $('goldSelect').value=current;
 }
 renderLibrary();renderGoldSearch();renderReaderProgress();
}
function init(){
 ...
 let st=state();if(st.lastGold&&CHAPTERS[st.lastGold])activeGold=st.lastGold;if(!CHAPTERS[activeGold])activeGold=Object.keys(CHAPTERS)[0];
 $('goldSelect').onchange=()=>openGold($('goldSelect').value,true);
 refreshCoreUI();renderGold();renderPatient();renderSources();
}
...
const sections=[['definition','Definition & scope','blue'],['finalClinicalMaster','FINAL Clinical Master','green'],['classification','Classification','blue'],['pathophysiology','Pathophysiology & root compression','blue'],['anatomy','Anatomy & localization','blue'],['etiology','Etiology / causes','blue'],['riskFactors','Risk factors / recovery modifiers','amber'],['presentation','Clinical presentation','blue'],['redFlags','RED FLAGS - urgent assessment','red'],['history','Focused history','blue'],['exam','Focused examination','blue'],['clinicalReasoning','Advanced clinical reasoning','amber'],['clinicalMaster','World-Class Clinical Master','green'],['rootMap','Practical root localization','blue'],['differentials','Differential diagnosis','amber'],['investigations','Investigations & imaging','blue'],['imagingCorrelation','Imaging-clinical correlation rules','amber'],['management','Management principles','green'],['rehab','Rehabilitation / graded return','green'],['outcomes','Outcome measures','blue'],['ayurvedaFramework','Ayurveda diagnostic framework','green'],['researchNote','Research note','amber']];
function list(v){return `<ul>${(v||[]).map(x=>`<li>${esc(x)}</li>`).join('')}</ul>`}

function renderReasoningLayer(R){
 if(!R)return '';
 let h=`<div class="reasoningHeader"><span class="eyebrow">MD / NEUROSPINE CLINICAL REASONING</span><h2>${esc(R.title||'Advanced clinical reasoning')}</h2></div>`;
 if(R.principles?.length)h+=`<h3>Core interpretation principles</h3>${list(R.principles)}`;
 if(R.slr?.length)h+=`<h3>SLR / Lasègue interpretation</h3><table class="reasonTable"><tr><th>Finding</th><th>Clinical meaning</th><th>What to do</th></tr>${R.slr.map(x=>`<tr><td><b>${esc(x.range)}</b></td><td>${esc(x.meaning)}</td><td>${esc(x.action)}</td></tr>`).join('')}</table>`;
 if(R.severity?.length)h+=`<h3>Clinical severity: do not use pain or SLR angle alone</h3><table class="reasonTable"><tr><th>Domain</th><th>Mild</th><th>Moderate</th><th>Severe / concerning</th><th>Interpretation</th></tr>${R.severity.map(x=>`<tr><td><b>${esc(x.domain)}</b></td><td>${esc(x.mild)}</td><td>${esc(x.moderate)}</td><td>${esc(x.severe)}</td><td>${esc(x.interpretation)}</td></tr>`).join('')}</table>`;
 if(R.localization?.length)h+=`<h3>Root localization matrix</h3><table class="reasonTable"><tr><th>Root</th><th>Common disc relationship</th><th>Pain</th><th>Sensory</th><th>Motor</th><th>Reflex</th><th>Gait / function</th><th>Clinical pearl</th></tr>${R.localization.map(x=>`<tr><td><b>${esc(x.root)}</b></td><td>${esc(x.commonDisc)}</td><td>${esc(x.pain)}</td><td>${esc(x.sensory)}</td><td>${esc(x.motor)}</td><td>${esc(x.reflex)}</td><td>${esc(x.gait)}</td><td>${esc(x.pearls)}</td></tr>`).join('')}</table>`;
 if(R.symptomCorrelation?.length)h+=`<h3>Symptom → meaning → next examination</h3><table class="reasonTable"><tr><th>Symptom</th><th>Meaning</th><th>Next step</th></tr>${R.symptomCorrelation.map(x=>`<tr><td><b>${esc(x.symptom)}</b></td><td>${esc(x.meaning)}</td><td>${esc(x.next)}</td></tr>`).join('')}</table>`;
 if(R.imagingDecision?.length)h+=`<h3>Imaging decision matrix</h3><table class="reasonTable"><tr><th>Clinical scenario</th><th>Investigation</th><th>Why</th><th>Priority</th></tr>${R.imagingDecision.map(x=>`<tr><td>${esc(x.scenario)}</td><td><b>${esc(x.test)}</b></td><td>${esc(x.why)}</td><td><span class="reasonPriority">${esc(x.priority)}</span></td></tr>`).join('')}</table>`;
 if(R.medEvidence?.length)h+=`<h3>Medication evidence matrix — common practice vs evidence</h3><table class="reasonTable"><tr><th>Medicine / class</th><th>Clinical role</th><th>Evidence interpretation</th><th>Status</th></tr>${R.medEvidence.map(x=>`<tr><td><b>${esc(x.drug)}</b></td><td>${esc(x.role)}</td><td>${esc(x.evidence)}</td><td><span class="reasonPriority">${esc(x.status)}</span></td></tr>`).join('')}</table>`;
 if(R.decisionPearls?.length)h+=`<h3>Clinical decision pearls</h3>${list(R.decisionPearls)}`;
 return h;
}
function mt(rows,cols){if(!rows?.length)return'';return `<div class="tableScroll"><table class="masterTable"><tr>${cols.map(c=>`<th>${esc(c[1])}</th>`).join('')}</tr>${rows.map(r=>`<tr>${cols.map(c=>`<td>${esc(r[c[0]]??'')}</td>`).join('')}</tr>`).join('')}</table></div>`}
function renderClinicalMaster(M){if(!M)return'';let h=`<div class="masterHero"><span class="eyebrow">WORLD-CLASS CLINICAL MASTER</span><h2>${esc(M.name)}</h2><p>${esc(M.oneLine||'')}</p></div>`;const b=(t,a)=>a?.length?`<details open><summary>${esc(t)}</summary>${list(a)}</details>`:'';h+=`<div class="masterGrid">${b('Definition & terminology',M.definition)}${b('Risk groups / occupation',M.riskGroups)}${b('History checklist',M.historyChecklist)}${b('Prognosis',M.prognosis)}${b('Quick recall',M.recall)}</div>`;if(M.anatomy)h+=`<details><summary>Anatomy & structures</summary>${mt(M.anatomy,[['structure','Structure'],['relevance','Clinical relevance']])}</details>`;if(M.epidemiology)h+=`<details><summary>Epidemiology & prevalence</summary>${mt(M.epidemiology,[['item','Domain'],['data','Data'],['interpretation','Interpretation']])}</details>`;if(M.pathophysiology)h+=`<details><summary>Pathophysiology</summary>${mt(M.pathophysiology,[['mechanism','Mechanism'],['detail','Clinical meaning']])}</details>`;if(M.physicalExamination)h+=`<details open><summary>Complete physical examination</summary>${M.physicalExamination.map(x=>`<h4>${esc(x.group)}</h4>${list(x.tests)}`).join('')}</details>`;if(M.specialTests)h+=`<details open><summary>Special tests — meaning & limitations</summary>${mt(M.specialTests,[['test','Test'],['positive','Positive finding'],['meaning','Meaning'],['limit','Limitation']])}</details>`;if(M.slrDegrees)h+=`<details open><summary>SLR angle interpretation</summary>${mt(M.slrDegrees,[['range','Range'],['interpretation','Interpretation'],['action','Clinical action']])}</details>`;if(M.rootLocalization)h+=`<details open><summary>Root localization</summary>${mt(M.rootLocalization,[['root','Root'],['disc','Disc relationship'],['pain','Pain'],['sensory','Sensory'],['motor','Motor'],['reflex','Reflex'],['function','Function']])}</details>`;if(M.differential)h+=`<details><summary>Differential diagnosis</summary>${mt(M.differential,[['condition','Condition'],['differentiate','How to differentiate']])}</details>`;if(M.labs)h+=`<details><summary>Laboratory investigations</summary>${mt(M.labs,[['test','Test'],['when','When'],['why','What it answers']])}</details>`;if(M.imagingTimeline)h+=`<details open><summary>Investigation timeline</summary>${mt(M.imagingTimeline,[['when','When'],['test','Investigation'],['why','Why']])}</details>`;if(M.xray)h+=`<details><summary>X-ray findings & limitations</summary>${mt(M.xray,[['finding','Finding'],['meaning','Meaning / limitation']])}</details>`;if(M.mri)h+=`<details open><summary>MRI findings & clinical correlation</summary>${mt(M.mri,[['finding','MRI finding'],['meaning','Clinical meaning']])}</details>`;if(M.otherTests)h+=`<details><summary>CT / PET / DEXA / EMG</summary>${mt(M.otherTests,[['test','Test'],['role','When / why']])}</details>`;if(M.treatmentTimeline)h+=`<details open><summary>Treatment timeline</summary>${mt(M.treatmentTimeline,[['stage','Stage'],['plan','Plan']])}</details>`;if(M.pharmacology)h+=`<details open><summary>Pharmacology — evidence & limitations</summary>${mt(M.pharmacology,[['class','Class'],['examples','Examples'],['role','Role'],['evidence','Evidence'],['cautions','Cautions']])}</details>`;if(M.interventions)h+=`<details><summary>Interventions</summary>${mt(M.interventions,[['option','Option'],['when','When'],['limit','Limitations']])}</details>`;if(M.surgeryDecision)h+=`<details open><summary>Conservative vs surgery decision</summary>${mt(M.surgeryDecision,[['pathway','Pathway'],['criteria','Criteria'],['action','Action']])}</details>`;if(M.lifestyle)h+=`<details><summary>Lifestyle / activity / diet — with WHY</summary>${mt(M.lifestyle,[['advice','Advice'],['why','Why']])}</details>`;if(M.sources)h+=`<details><summary>Primary source map</summary><ul>${M.sources.map(s=>`<li><b>${esc(s.org)}</b> — ${esc(s.title)}</li>`).join('')}</ul></details>`;return h}

function ftable(rows,cols){
 if(!rows?.length)return '';
 return `<div class="tableScroll"><table class="finalTable"><tr>${cols.map(c=>`<th>${esc(c[1])}</th>`).join('')}</tr>${rows.map(r=>`<tr>${cols.map(c=>`<td>${esc(r[c[0]]??'')}</td>`).join('')}</tr>`).join('')}</table></div>`;
}
function finalBullets(title,a,open=false){
 if(!a?.length)return '';
 return `<details ${open?'open':''}><summary>${esc(title)}</summary>${list(a)}</details>`;
}
function renderFinalMaster(M){
 if(!M)return '';
 let h=`<div class="finalHero"><span class="eyebrow">FINAL CLINICAL BENCHMARK</span><h2>${esc(M.name)}</h2><p>${esc(M.oneLine)}</p><div class="memoryStrip">${(M.memoryMap||[]).map(x=>`<span>${esc(x)}</span>`).join('')}</div></div>`;
 h+=finalBullets('Definition & core terminology',M.definition,true);
 h+=`<details><summary>Terminology / synonyms</summary>${ftable(M.terminology,[['term','Term'],['meaning','Meaning']])}</details>`;
 h+=`<details open><summary>Applied anatomy</summary>${ftable(M.appliedAnatomy,[['structure','Structure'],['clinical','Clinical relevance']])}</details>`;
 h+=`<details><summary>Epidemiology</summary>${ftable(M.epidemiology,[['domain','Domain'],['data','Evidence summary'],['takeHome','Take-home']])}</details>`;
 h+=`<details><summary>Occupation & risk factors</summary>${ftable(M.occupationalRisk,[['exposure','Exposure'],['evidence','Evidence'],['clinical','Clinical use']])}</details>`;
 h+=finalBullets('Etiology / causes',M.etiology);
 h+=`<details><summary>Pathophysiology</summary>${ftable(M.pathophysiology,[['mechanism','Mechanism'],['explanation','Meaning']])}</details>`;
 h+=`<details><summary>Clinical patterns / phenotypes</summary>${ftable(M.clinicalPatterns,[['pattern','Pattern'],['clues','Clinical clues']])}</details>`;
 h+=finalBullets('Focused history — complete checklist',M.history,true);
 h+=`<details open class="redDetail"><summary>Red flags & urgent action</summary>${ftable(M.redFlags,[['flag','Red flag'],['meaning','What it may mean'],['action','Action']])}</details>`;
 h+=`<details open><summary>Physical examination — step-by-step</summary>${ftable(M.examSequence,[['step','Sequence'],['items','What to examine']])}</details>`;
 h+=`<details open><summary>Special tests — technique + interpretation</summary>${ftable(M.specialTests,[['test','Test'],['how','How'],['positive','Positive finding'],['meaning','Meaning'],['limitation','Limitation'],['next','Next decision']])}</details>`;
 h+=`<details open><summary>SLR angle — what it means and what it DOES NOT mean</summary>${ftable(M.slrAngleLogic,[['angle','Angle'],['interpretation','Interpretation'],['severity','Severity rule'],['next','Next step']])}</details>`;
 h+=`<details><summary>Neurological severity</summary>${ftable(M.neurologicSeverity,[['domain','Domain'],['low','Lower concern'],['intermediate','Intermediate'],['high','High concern'],['meaning','Interpretation']])}</details>`;
 h+=`<details open><summary>L4 / L5 / S1 localization</summary>${ftable(M.rootLocalization,[['root','Root'],['disc','Common disc relationship'],['pain','Pain'],['sensory','Sensory'],['motor','Motor'],['reflex','Reflex'],['function','Function'],['special','Pearl']])}</details>`;
 h+=`<details><summary>Symptom → localization reasoning</summary>${ftable(M.symptomToLocalization,[['symptom','Symptom'],['meaning','Meaning'],['do','What to examine']])}</details>`;
 h+=`<details><summary>Differential diagnosis</summary>${ftable(M.differential,[['condition','Condition'],['clue','How to separate']])}</details>`;
 h+=`<details><summary>Laboratory tests</summary>${ftable(M.labTests,[['test','Test'],['when','When'],['helps','What it helps answer']])}</details>`;
 h+=`<details open><summary>Investigation timeline</summary>${ftable(M.investigationTimeline,[['scenario','Scenario'],['first','First test'],['next','Next'],['reason','Why']])}</details>`;
 h+=`<details><summary>X-ray — findings & limitations</summary>${ftable(M.xray,[['finding','Finding'],['meaning','Meaning / limitation']])}</details>`;
 h+=`<details open><summary>MRI — common findings & clinical correlation</summary>${ftable(M.mri,[['finding','MRI finding'],['clinical','Clinical correlation']])}</details>`;
 h+=`<details><summary>CT / CT myelography / DEXA / PET / EMG-NCS</summary>${ftable(M.otherImaging,[['test','Test'],['when','When / why'],['limitation','Limitation']])}</details>`;
 h+=`<details open><summary>Conservative-treatment timeline</summary>${ftable(M.conservativeTreatment,[['stage','Stage'],['goal','Goal'],['plan','Plan']])}</details>`;
 h+=`<details open><summary>Medication evidence matrix</summary>${ftable(M.drugMatrix,[['class','Drug/class'],['examples','Examples'],['role','Role'],['evidence','Evidence'],['safety','Safety'],['status','Status']])}</details>`;
 h+=`<details><summary>Interventions / procedures</summary>${ftable(M.interventions,[['option','Option'],['indication','Indication'],['expected','Expected benefit'],['avoid','Limitation / avoid']])}</details>`;
 h+=`<details open><summary>Conservative vs surgery decision</summary>${ftable(M.surgeryDecision,[['path','Path'],['criteria','Criteria'],['action','Action']])}</details>`;
 h+=finalBullets('Prognosis',M.prognosis);
 h+=`<details><summary>Follow-up timeline</summary>${ftable(M.followUp,[['time','When'],['who','Who'],['check','What to reassess']])}</details>`;
 h+=finalBullets('Outcome measures',M.outcomeMeasures);
 h+=`<details><summary>Lifestyle / activity / diet — with WHY</summary>${ftable(M.lifestyle,[['advice','Advice'],['why','Why']])}</details>`;
 h+=finalBullets('Rapid recall — one-read memory points',M.rapidRecall,true);
 h+=`<details><summary>Source map</summary>${ftable(M.sourceMap,[['source','Source'],['supports','What it supports']])}</details>`;
 return h;
}
