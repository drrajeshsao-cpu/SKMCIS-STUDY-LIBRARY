...
let INDEX=[],CHAPTERS={},activeGold='SKM-SPN-0001',PRESCRIBER={},REASONING={};let deferredPrompt=null;
...
async function load(){
 ...
 try{
   ...
   Object.keys(REASONING).forEach(id=>{if(CHAPTERS[id]){CHAPTERS[id].clinicalReasoning=REASONING[id];CHAPTERS[id].clinicalReasoning && (CHAPTERS[id].clinicalReasoningLoaded=true)}});
   Object.keys(REASONING).forEach(id=>{if(CHAPTERS[id]&&REASONING[id])CHAPTERS[id].clinicalReasoning=REASONING[id]});
 }catch(e){console.warn('Clinical reasoning load failed',e)}

 // Add a renderable property only where reasoning exists.
 Object.keys(CHAPTERS).forEach(id=>{if(CHAPTERS[id].clinicalReasoning)CHAPTERS[id].clinicalReasoning=CHAPTERS[id].clinicalReasoning});
 init();
